...
int postfixStackTop = -1;//-1로 값 설정
int evalStackTop = -1;

int evalResult = 0;//초기화
...
precedence getToken(char symbol)//토큰 입력받기
{
	switch(symbol) {
	case '(' : return lparen;
	case ')' : return rparen;
...
void toPostfix()
{
	...
	while(*exp != '\0')
	{
        ...
        else if(getPriority(*exp) == lparen) {// ( 이면
        ...
        {
            while(getPriority(postfixStack[postfixStackTop]) >= getPriority(*exp))//postfixStack이 크면
            {
            	x = postfixPop();//팝
            	charCat(&x);
            }
    ...
    while(postfixStackTop != -1)//-1이 아니면
    {
    	x = postfixPop();
    	charCat(&x);
    }
...
void evaluation()//계산
{
	...
	int length = strlen(postfixExp);//길이
	char symbol;
	evalStackTop = -1;
